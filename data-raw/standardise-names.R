...
if(!file.exists("data/taxonomyStd.rda")){
  ...
  taxonomyStd <- traitdata::standardize_taxonomy(x=data_names,
                                                 return=c("kingdom",
                                                          "phylum",
                                                          "class",
                                                          "order",
  ...
  if(nrow(names_new) > 0){
    ...
    names_new <- traitdata::standardize_taxonomy(x=names_new,
...
synonyms <- inner_join(synonyms, taxon_sub, by="scientificName") %>% select(-scientificNameStd)
