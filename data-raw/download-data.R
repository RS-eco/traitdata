...
heteropteraRaw <- tidyr::separate(heteropteraRaw, SpeciesID, into=c("Genus", "Species"), sep=" ", remove=T, extra="drop")
...
#migbehav_birds <- utils::read.csv("http://141.2.226.7/database/metacat?action=read&qformat=bikf&sessionid=&docid=bikf.10056.1",
#                                  sep = "\t")
...
amphi_lifehist  <- tidyr::separate(amphi_lifehist, Species, into=c("Genus", "Species"), sep=" ", extra="drop")
#temp <- tempfile(fileext = ".xlsx")
...
attr(amphi_lifehist, 'citeAs') <- utils::bibentry(
  ...
  volume = 2,
  issue = "e4123",
  author = c(utils::person(given = "Audrey", family = "Trochet", email = "audrey.trochet@univ-tlse3.fr")
  ),
  year = 2014,
...
mammal_diet2 <- dplyr::select(mammal_diet2, -binomial)
unlink(temp)
