...
passerines %>% group_by(Family) %>% tidyr::drop_na(Family) %>% 
  ...
  arrange(desc(n)) %>% head(10) %>% collect() %>% ggplot(aes(x=Family, y=n)) +
...
passerines %>% group_by(scientificNameStd, Sex) %>% filter(Sex %in% c(1,2)) %>% 
  summarise(wing_length = mean(Wing), 
            sample_size = n()) %>% head(10)

## ----fig.width=6, fig.height=4------------------------------------------------
passerines %>% group_by(scientificNameStd, Sex) %>% filter(Sex %in% c(1,2)) %>% 
  head(20) %>% collect() %>% ggplot(aes(x=as.factor(`scientificNameStd`), y=Wing, fill=Sex)) + geom_boxplot() + coord_flip() + 
